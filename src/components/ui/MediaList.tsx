...
export default function MediaList(): React.JSX.Element {
  ...
  const handleAccordionChange = (values: string[]) => {
    ...
    if (values.length < prevOpenItems.current.length && latestOpened.current && !values.includes(latestOpened.current)) {
      setMood("idle");
      setMessage(null);
            ...
            {items.map((item) => (
              <AccordionItem key={item.id} value={`item-${item.id}`}>
                ...
                <AccordionContent className="flex flex-col gap-4 text-balance">
                  <VideoPlayer id={item.id} src={item.videoUrl} title={item.title} posterSrc={item.thumbnailUrl} autoPlay={openItems.includes(`item-${item.id}`)} />
                  <div>CONTENT</div>
                </AccordionContent>
              </AccordionItem>
            ))}
